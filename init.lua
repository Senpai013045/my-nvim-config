...
if vim.fn.executable("tree-sitter") ~= 1 then
	if vim.fn.executable("npm") == 1 then
		...
		pcall(function()
			vim.system({ "npm", "install", "--prefix", vim.fn.stdpath("data") .. "/tree-sitter-cli", "tree-sitter-cli" })
				:wait()
		end)
	end
end
...
pcall(function()
	require("nvim-treesitter").install({ "c", "javascript", "typescript", "tsx", "html", "css", "json", "markdown" })
end)
...
lint.linters.eslint_d = require("lint.util").wrap(lint.linters.eslint_d, function(diag)
	if diag.message:find("Error: Could not find config file") then
		return nil
	end
	return diag
end)
...
require("gitsigns").setup({
	...
	on_attach = function(bufnr)
		...
		end, { desc = "git hunk [Q]uickfix list" })
